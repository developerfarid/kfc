    ...
    useEffect(() => {
        async function fetchData() {
            ...
            $.ajax({
                ...
                url: window.BACK_END_URL + 'coachAndRefList.php',
                cache: false
            });
        }
        fetchData();
    ...
    return (
        <>
            <div className='bg-white h-24 flex justify-between px-8 items-center fixed top-0 left-3/12 right-0 w-9/12'>
                <h2 className='text-lg'>Registration  <span className='text-gray-600'>(Referees)</span></h2>
                <IoMdNotificationsOutline className='text-lg' />
            </div>
            <div className='mt-24'>
                <div className='flex justify-end mb-8 px-8 pt-8'>

                    <Link to='/Dashboard/Referee/0' className='btn-update'><IoIosAddCircleOutline className=' mr-3 text-2xl' /> Add Referee</Link>
                </div>
                <div className='px-8'>
                                ...
                                <table className="table-auto text-center">
                                    <thead className='pb-3'>
                                        <tr>
                                            ...
                                            <th className='pb-3 text-sm md:text-lg font-semibold'>Referee Name</th>
                                            <th className='pb-3 text-sm md:text-lg font-semibold'>Date of Birth</th>
                                            <th className='pb-3 text-sm md:text-lg font-semibold'>Salary</th>
                                            <th className='pb-3 text-sm md:text-lg font-semibold'>City</th>
                                    ...
                                    <tbody className=''>
                                        ...
                                        {isDataPresent && (dataArray.map((item, index) => (
                                            <><tr className='bg-bg-body' key={item.id}>
                                                ...
                                                <td className='py-3 my-2'>{item.dob}</td>
                                                <td className='py-3 my-2'>{item.salary}</td>
                                                <td className='py-3 my-2'>{item.city}</td>
                                                <td className='py-3 my-2 rounded-r-lg '>
                                                    <Link to={`/Dashboard/Referee/${item.id}`}><AiOutlineEdit className='text-center inline-block' /></Link>
                                                </td>
                                            </tr>
