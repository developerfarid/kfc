        ...
        <>
            ...
            <section className='py-8'>
                <div className='container'>
                    <div className='flex justify-between items-center py-5'><h3 className='text-xl md:text-4xl font-semibold'>{t("common.Referee")} </h3>   <div className="relative w-32 md:w-auto text-gray-600 border rounded-lg   focus-within:text-gray-400">
                        <span className="absolute inset-y-0 left-0  flex items-center ltr:pl-2 rtl:pr-2">
                            <button type="submit" className="p-1  focus:outline-none focus:shadow-outline">
                                <svg fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" className="w-5 h-5 md:w-6 md:h-6"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                            </button>
                        </span>
                        <input type="search" name="q" className="py-3 md:py-4 text-sm text-white  rounded-md ltr:pl-10 rtl:pr-10 focus:outline-none placeholder:text-sm md:placeholder:text-base focus:bg-white focus:text-gray-900" placeholder={t("news.Search")} autoComplete="off" />
                    </div>
                    </div>
                        ...
                        dataArray.map((item, i) => (
                                <div className='mb-5 border-2 rounded-lg' key={i}>
                                    <div className='relative -mb-24 before:absolute before:left-0 before:top-0 before:w-full before:h-full overflow-hidden rounded-t-lg '>
                                        <img  className='w-full h-[175px] blur-sm' src={img} alt="" />
                                        <span className='absolute flex justify-center items-center right-4 rtl:left-4 top-4 w-[36px] h-[36px] rounded-full p-1 text-white bg-[#879ba9]  text-xl'>
                                    ...
                                    <Link to={`/RefereeInfo/${item.id}`} state={{ data: item }}> <img className="w-36   inline-block  transform object-cover  h-36 rounded-full l text-center mx-auto border-2 border-white"  src={`${window.IMG_HOST_URL}${item.img_path}`} alt="" /></Link>
                                    </div>
                                    <div className='py-5 px-5 flex justify-between items-end '>
                                    <div className='space-y-3'>
                                        <h3 className='text-2xl font-semibold'> <Link to={`/RefereeInfo/${item.id}`} state={{ data: item }}> {item.name}</Link></h3>
                                        <p className='flex items-center text-[#515151] text-[18px]'>
                                            {t('request.Football')} : <span className=' font-semibold'> {t("common.Referee") }</span>
                                        </p>
                                            <p className='flex items-center text-[#515151] text-[18px]'>
                                           {t("Footer.Match")}  :  <span className='font-semibold'>  { item.totalmatch}</span>
